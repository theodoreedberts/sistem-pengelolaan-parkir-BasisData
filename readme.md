...
[cite_start]Proyek ini merupakan implementasi **Sistem Pengelolaan Parkir Berbasis Data Relasional** yang dirancang untuk mengatasi problem antrean fisik, risiko kebocoran pendapatan, serta manipulasi pencatatan denda manual[cite: 17]. [cite_start]Sistem ini mendukung kalkulasi tarif progresif otomatis, integrasi status keanggotaan (member), manajemen denda, hingga pelacakan kapasitas slot parkir secara real-time[cite: 18, 19, 46].

> [cite_start]Proyek ini disusun sebagai Tugas Besar mata kuliah IN224T - Basis Data (Semester Genap 2025/2026), Program Studi Teknik Informatika, Universitas Kristen Maranatha[cite: 2, 11, 12].

---

## 👥 Anggota Tim
1. [cite_start]**Theodore Edbert Suryo** (2572002) [cite: 10]
2. [cite_start]**Kenneth Ansell Hansjaya** (2572013) [cite: 10]
3. [cite_start]**Darren Nicholas** (2572019) [cite: 10]
...
1. [cite_start]**Area Parkir (`Area_Parkir`)**: Mengatur zonasi (Mobil/Motor) dan batas total daya tampung armada[cite: 83, 85].
2. [cite_start]**Jenis Tarif (`Jenis_Tarif`)**: Menyediakan acuan biaya per jam berdasarkan kategori (Reguler/Member/VIP) tanpa mengganggu logika aplikasi jika ada perubahan tarif[cite: 90, 91, 92].
3. [cite_start]**Petugas (`Petugas`)**: Menyimpan data kredensial staf kasir/operator untuk menjaga akuntabilitas *audit trail*[cite: 94, 95].
4. [cite_start]**Denda (`Denda`)**: Master data penalti (seperti tiket hilang, parkir sembarangan, atau menginap tanpa izin)[cite: 97, 98].
5. [cite_start]**Member (`Member`)**: Melacak data validitas kontrak tenggang waktu keanggotaan pelanggan[cite: 100, 101, 102].
6. [cite_start]**Kendaraan (`Kendaraan`)**: Entitas fisik armada (dibuat *nullable* pada `id_member` agar fleksibel untuk kendaraan non-member)[cite: 103, 104, 105, 152].
7. [cite_start]**Transaksi (`Transaksi`)**: Log masuk-keluar kendaraan, lengkap dengan penanda status aktif dan bukti visual foto gerbang[cite: 108, 109, 110].
8. [cite_start]**Pembayaran (`Pembayaran`)**: Bukti digital pelunasan finansial yang mengunci pembukuan akhir pendapatan[cite: 112, 113, 114].
